...
export const SlashCommandPicker: React.FC<SlashCommandPickerProps> = ({
  ...
  useEffect(() => {
    ...
    if (!query) {
      ...
      filtered = filteredByTab.filter((cmd) => {
        ...
        if (cmd.namespace && cmd.namespace.toLowerCase().includes(query))
          return true;

        // Match against description
        if (cmd.description && cmd.description.toLowerCase().includes(query))
          return true;
  ...
  const groupedCommands = filteredCommands.reduce((acc, cmd) => {
    ...
    if (cmd.scope === "user") {
      ...
      key = cmd.namespace
        ? `Project Commands: ${cmd.namespace}`
        : "Project Commands";
    } else {
                ...
                {filteredCommands.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-full">
                    ...
                    {!searchQuery && (
                      <p className="text-xs text-muted-foreground mt-2 text-center px-4">
                        Create commands in{" "}
                        <code className="px-1">.claude/commands/</code> or{" "}
