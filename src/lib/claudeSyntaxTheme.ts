...
// Dynamic theme getter that always returns current theme
export const getClaudeSyntaxThemeAuto = () => {
  // Check if we're in a browser environment and if document has dark class
  if (typeof document !== "undefined") {
    const isDark = document.documentElement.classList.contains("dark");
    return isDark ? claudeSyntaxThemeDark : claudeSyntaxThemeLight;
  }
  // Fallback to light theme for SSR
  return claudeSyntaxThemeLight;
};

// Default export for backward compatibility - gets current theme dynamically
export const claudeSyntaxTheme = getClaudeSyntaxThemeAuto();
...
// Hook to automatically get current syntax theme based on active theme
export function useClaudeSyntaxTheme() {
  ...
  useEffect(() => {
    ...
    const updateTheme = () => {
      const isDark = document.documentElement.classList.contains("dark");
      setCurrentTheme(isDark ? claudeSyntaxThemeDark : claudeSyntaxThemeLight);
    };
